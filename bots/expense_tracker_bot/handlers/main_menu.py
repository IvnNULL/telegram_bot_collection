...
from keyboards.keyboards import get_main_menu_keyboard, get_show_menu_kb, get_back_to_menu_kb
from services.expense_service import expenses_to_csv_text, text_to_csv_file, expenses_to_text
...
@menu_router.callback_query(MenuCallback.filter(F.target == 'show_all'))
async def process_show_all_click(callback: CallbackQuery, session: AsyncSession):
    await callback.answer()
    with suppress(TelegramBadRequest):
        await callback.message.delete()
    ...
    await callback.message.answer(text=text, reply_markup=get_back_to_menu_kb())
